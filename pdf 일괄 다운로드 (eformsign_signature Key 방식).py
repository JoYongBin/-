import os #운영체제 상호작용
import re #정규 표현식(Regular Expression), 문자열 매칭 사용
import requests #API 호출 및 서버 통신 (설치 필요)
import pandas as pd #데이터 분석 (설치 필요)
import openpyxl
import json
...
def document_list():
    ...
    data = {
        ...
        "start_update_date": "1640995200000",  # 시작 시간              *주의 사항* : 날짜 단위를 너무 길게 잡으면 504 error(Server Timeout), 해당 시간은 GPT Timestamp in seconds 검색 참고.
        "end_update_date": "1734393600000"  # 종료 시간                 *주의 사항* : 날짜 단위를 너무 길게 잡으면 504 error(Server Timeout), 해당 시간은 GPT Timestamp in seconds 검색 참고.
    }
